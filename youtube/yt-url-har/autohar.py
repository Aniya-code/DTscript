...
chrome_options.add_experimental_option("prefs", {
	'download.default_directory': os.path.join(CUR_ABS_PATH, 'har_result'),  # 设置默认下载路径
})
...
if __name__ == '__main__':
    ###########修改默认分辨率 1.here  2.content.js   3.修改输入法ENG###########
    RESOLUTION = '720'
    URL_LIST = [
        ...
        # 'https://www.youtube.com/watch?v=CPNva8Wcc3g',
        ...
        # 'https://www.youtube.com/watch?v=-NtSZQHRc-I',
        # 'https://www.youtube.com/watch?v=1dr8S65l_T8',
        # 'https://www.youtube.com/watch?v=37dc5a_hBwA',
        # 'https://www.youtube.com/watch?v=41vCAtuv0gw',
        # 'https://www.youtube.com/watch?v=47ntBElzaWk',
        # 'https://www.youtube.com/watch?v=5OtdPi4ecA4',
        # 'https://www.youtube.com/watch?v=6KRl0lg7XmQ',
        # 'https://www.youtube.com/watch?v=AmVRXThL0Xk',
        # 'https://www.youtube.com/watch?v=A_aAyHNzpTw',
        # 'https://www.youtube.com/watch?v=BGawxtM4X7U',
    ]
    ########################################################

    # main
    # tqdm也可以套在enumerate外面，但是这样需要手动指定total值才有进度条，因为枚举返回的是迭代器，没有len。
    # 最简单的写法就是直接套在支持len的容器上。对于迭代器，不指定total参数值，只会打印第i次迭代，无进度条。
    for i, url in enumerate(tqdm(URL_LIST, desc="总进度")):
        ...
        wait_second, waste_second2 = get_wait_second()
        if (sleep_time:=wait_second - waste_second1 - waste_second2) > 0:
            time.sleep(sleep_time)
        
        _dir = os.path.join(CUR_ABS_PATH, "har_result", url[-11:])
        if not os.path.exists(_dir):
            os.mkdir(_dir)
        export_har_file(dir_name=url[-11:], file_name=f"{url[-11:]}--{RESOLUTION}--{datetime.now():%Y%m%d%H%M%S}.har")

        # ensure the save is successful (the save process took much time)
        time.sleep(wait_second / 60 / 2) # 10min -> 5s  60min -> 30s
