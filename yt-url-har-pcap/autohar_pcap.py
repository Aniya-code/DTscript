...
if __name__ == '__main__':
    ...
    TSHARK_PATH = 'D:/App/Wireshark/tshark.exe'
    INTERFACE = '以太网'
    ########################################################
    ...
    for i, url in enumerate(tqdm(URL_LIST, desc="总进度")):
        ...
        ensure_tshark_gen = capture_traffic(TSHARK_PATH, INTERFACE, os.path.join(DEFAULT_DOWN_PATH, f'{url[-11:]}--PCAP'), f'{traffic_basefilename}.pcap')
        next(ensure_tshark_gen)  # ensure tshark started
        print('Tshark Started')
        
        driver.get(url) # 阻塞，直到网页加载完成
